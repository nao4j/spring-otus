...
import ru.yegorpilipenko.otus.spring.m2h1.service.AuthorService

@ShellComponent
class AuthorController(val authorService: AuthorService) {

    @ShellMethod(value = "Show all authors.", group = "Author Commands")
    fun authors(): Collection<String> = authorService.findAll().map { author -> "${author.id}: ${author.firstName} ${author.lastName}" }
